...
extern void handle_label_def(std::shared_ptr<ASTNode>& node, Parser& p, SymTable& table, const Token& tok);
extern void handle_sym(std::shared_ptr<ASTNode>& node, Parser& p, SymTable& table, const Token& tok);
extern std::shared_ptr<ASTNode> processOpCodeRule(RULE_TYPE ruleType,
    const std::vector<RuleArg>& args, Parser& p, int count);
extern std::shared_ptr<ASTNode> processOpCodeRule(std::vector<RULE_TYPE> rule,
